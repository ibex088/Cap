...
window.addEventListener('message', (event) => {
  if (event.data.type === 'RECORDING_STARTED') {
    ...
    onRecordingStopped();
  } else if (event.data.type === 'UPLOAD_PROGRESS') {
    updateUploadProgress(event.data.progress, event.data.status);
  } else if (event.data.type === 'CAP_DEVICES_ENUMERATED') {
    handleDevicesEnumerated(event.data.cameras, event.data.mics);
  } else if (event.data.type === 'CAP_PERMISSION_RESPONSE') {
    handlePermissionResponse(event.data);
  }
});
...
function handlePermissionResponse(data) {
  const { messageId, granted, devices } = data;
    ...
    if (result.granted) {
      if (kind === 'camera') {
        ...
        if (result.devices.length > 0) {
          availableCameras = result.devices;
          updateDeviceSelectors();
        }
      } else {
        micPermissionState = 'granted';
        if (result.devices.length > 0) {
          availableMics = result.devices;
          updateDeviceSelectors();
        }
      }
      updatePermissionUI();
...
function handleDevicesEnumerated(cameras, mics) {
...
function setupEventListeners() {
  ...
  elements.cameraSelectBtn.addEventListener('click', () => {
    ...
    setTimeout(updateIframeHeight, 10);
  });
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!e.target.closest('.device-selector')) {
      const wasOpen = !elements.cameraDropdown.classList.contains('hidden') || !elements.micDropdown.classList.contains('hidden');
      elements.cameraDropdown.classList.add('hidden');
      elements.cameraSelectBtn.classList.remove('open');
      elements.micDropdown.classList.add('hidden');
      elements.micSelectBtn.classList.remove('open');
